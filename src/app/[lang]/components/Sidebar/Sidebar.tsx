...
const CabinetSideBar = ({ sidebar }: { sidebar: SideBarProps[] }) => {
    ...
    return (
        <header id="header" className={`bg-[var(--dark)] relative px-4 py-2 p-lg-4 overflow-y-auto scroll-bar-none ${isSidebarOpen ? "active" : ""}`}>
            ...
            <div onClick={() => setIsSidebarOpen((prev) => !prev)} className="absolute top-5 end-5 w-[34px] h-[34px] rounded-full bg-[var(--whiLg)] dark:bg-[var(--darkBorderCl)] flex items-center justify-center block sm:hidden cursor-pointer">
                {

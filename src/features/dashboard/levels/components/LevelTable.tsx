...
import { LevelTableProps } from "src/types/component";
            ...
            <div className="min-w-[992px]">
                    ...
                    <div className="col-span-2 flex items-center gap-4">
                        <label className="flex items-center cursor-pointer gap-2">
                            <input
                                ...
                                onChange={(e) => e.target.checked ? setChecked(levels.map((c) => c.id)) : setChecked([])}
                            />
                ...
                <div className="w-full h-[1px] bg-[var(--whiLg)] dark:bg-[var(--darkBorderCl)] mt-4" />
                {
                    levels && levels.map((level) => (
                        <div key={level.id} className="w-full grid grid-cols-5 items-center gap-4 cursor-pointer py-1">
                                ...
                                <div className="flex items-center gap-3">
                                    <label className="flex items-center cursor-pointer gap-2">
                                        <input
                                            ...
                                            onChange={(e) => setChecked(e.target.checked ? [...checked, level.id] : checked.filter((c) => c !== level.id))}
                                        />
                                        <div className="check-icon-bg min-w-6 min-h-6 border border-[var(--whiLg)] dark:border-[var(--darkInputBorder)] rounded-md flex items-center justify-center peer-checked:bg-[var(--primary)] transition-all duration-200">
                                            <FaCheck className="check-icon text-[var(--whi)] size-4" />
                                        </div>
                            ...
                            <div className="col-span-1">
                                <div className="flex items-center justify-end gap-1">
                                    <div onClick={() => onDelete([level.id])} className="item w-9 h-9 rounded-full hover:bg-[var(--whiLg)] hover:dark:bg-[var(--darkBorderCl)] flex items-center justify-center" >
                                        <FaTrashCan className="size-4 text-[var(--textCl)] dark:text-[var(--darkTextCl)]" />
                                    </div>
                                    <div onClick={() => onEdit(level)} className="item w-9 h-9 rounded-full hover:bg-[var(--whiLg)] hover:dark:bg-[var(--darkBorderCl)] flex items-center justify-center">
